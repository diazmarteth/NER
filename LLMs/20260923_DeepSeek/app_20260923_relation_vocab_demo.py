"""
Rococo NER Pipeline - SECURE Web Interface (DeepSeek API Edition)
...
4. (Optional) Set DEEPSEEK_BASE_URL to override the default DeepSeek endpoint

NOTE: unlike the local-Ollama edition, this sends document text to
DeepSeek's hosted API — it leaves this machine and incurs API cost.
"""
...
from rococo_ner_pipeline_20260923_relation_vocab_demo import RococoNERPipeline
...
<p class="institution-tag">Chair for History and Theory of Architecture · D-ARCH ETH · DeepSeek API Edition</p>

**Powered by:** DeepSeek API (hosted) · DeepSeek-Chat (DeepSeek-V3) · Tesseract OCR

**Security:** Document text is sent to DeepSeek's hosted API for analysis · 50 MB max &nbsp;|&nbsp;
...
HELP_MD = """
## **DeepSeek API Edition — with Page References**
...
<strong>Backend:</strong> DeepSeek API (hosted) + DeepSeek-Chat &nbsp;|&nbsp;
<strong>Rate limit:</strong> 10/hour (protects API budget & rate limits) &nbsp;|&nbsp;
...
class SecureRococoUI:
    """Secure web interface with rate limiting — DeepSeek API edition, with page refs."""

    def __init__(self):
        # RococoNERPipeline reads DEEPSEEK_API_KEY (and optionally
        # DEEPSEEK_BASE_URL) from the environment — set it before launching:
        #   export DEEPSEEK_API_KEY="sk-..."
        ...
        print("Secure DeepSeek API pipeline initialised (with page references)")
...
def main():
    ...
    print("Launching Rococo NER Pipeline (DeepSeek API Edition — with Page Refs)...")
    print("Make sure DEEPSEEK_API_KEY is set in the environment before launching.")
    ui = SecureRococoUI()
